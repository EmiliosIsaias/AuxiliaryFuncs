...
for cm = multiMice(:)' 
    ...
    function paddedMat = padMatrix(mat)
        switch ndims(mat{1})
            case 2
                paddedMat = arrayfun(@(x) ...
                    cellfun(@(m) ...
                    padarray(m, mxC{x} - size(m), NaN, "post"), ...
                    mat(expTypeMbrshp == x), fnOpts{:}), ...
                    unique(expTypeMbrshp), fnOpts{:});
                paddedMat = cellfun(@(x) cat(3, x{:}), paddedMat, fnOpts{:});
            case 3
                paddedMat = arrayfun(@(x) ...
                    cellfun(@(m) ...
                    padarray(m, [flip(mxC{x}), 8] - size(m), NaN, "post"), ...
                    mat(expTypeMbrshp == x), fnOpts{:}), ...
                    unique(expTypeMbrshp), fnOpts{:});
                paddedMat = cellfun(@(x) cat(4, x{:}), paddedMat, fnOpts{:});
        end
    end
...
homMiceMatTP = padMatrix( miceMatsTP );
homMiceMatAI = padMatrix( miceMatsAI );
homMiceMatPT = padMatrix( miceMatsPT );
homMiceMatPA = padMatrix( miceMatsPA );
...
% Padding rows of the condition names
homMiceConds = arrayfun(@(m,x) ...
    [miceConds{m}; strings([mxC{x}(1)-Nc_s(m,1), mxC{x}(2)] ) ], ...
    (1:Nm)', expTypeMbrshp(:), fnOpts{:});
...
xmice = struct('ExperimentalGroup', cellstr(expTypes(:)), ...
    'TrialProportions', homMiceMatTP(:), ...
    'AmplitudeIndex', homMiceMatAI(:), ...
    'PolygonUnfoldTrialProp', homMiceMatPT(:), ...
    'PolygonUnfoldAmplIndx', homMiceMatPA(:), ...
